...
import json
import logging
from typing import Dict, Optional, Set, Tuple, cast, List, Type
...
class ProcessRequestRound(CollectDifferentUntilAllRound, ChatCompletionABCIAbstractRound):
    """ProcessRequestRound"""
    ...
    def end_block(self) -> Optional[Tuple[BaseSynchronizedData, Enum]]:
        ...
        if len(self.collection) == len(self.synchronized_data.all_participants):
            ...
            if self.collection.values():
                ...
                existing_requests = [request for request in existing_requests if request["id"] not in processed_request_ids]
...
class SynchronizeRequestsRound(CollectDifferentUntilAllRound, ChatCompletionABCIAbstractRound):
    ...
    def end_block(self) -> Optional[Tuple[BaseSynchronizedData, Enum]]:
        ...
        if self.collection_threshold_reached:
            ...
            existing_responses = cast(SynchronizedData, self.synchronized_data).responses
            ...
            if new_requests:
                ...
                new_requests = [request for request in new_requests if request["id"] not in list(existing_ids)]
            ...
            existing_requests = [request for request in existing_requests if request["num_tries"] < 3]
